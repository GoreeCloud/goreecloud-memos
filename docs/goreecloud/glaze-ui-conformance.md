...
Physical Android acceptance showed that the preceding density refinement made the interface feel visually too small even though its spacing and interaction targets were sound. The final Compact cascade therefore raises the effective phone reading base to **20 CSS pixels** at 599 pixels and below while retaining the denser card, hero, and composer spacing. Home hero/supporting text, the quick-capture prompt, memo body text, and leading memo titles are correspondingly raised without restoring the earlier excessive vertical padding. Density and readability are treated as separate controls.
...
Android session and launch behavior also follows a mobile-specific resilience contract. A 15-minute access token expiring while the app is idle does not itself terminate the login: the existing 30-day rotating refresh session remains authoritative. Foreground recovery listens to visibility, focus, page-show, and online signals. Temporary network, timeout, unavailable, or server failures preserve the stored session and retry later; only an explicit server `Unauthenticated` refresh result is treated as proof that the refresh session is no longer valid. Cold start no longer presents a blank webview: both the bundled Tauri shell and the web/React application provide an immediate Glaze first-paint surface while the canonical HTTPS Memos origin and identity/profile requests settle.
...
- an effective 20-pixel Compact reading base on phones while preserving compact spacing and the existing practical interaction minimum;
...
- mobile session recovery that preserves a valid refresh session across transient background/wake connectivity failures and only signs out on a definitive authentication rejection;
- foreground refresh coverage for `visibilitychange`, `focus`, `pageshow`, and `online` lifecycle signals without background polling;
- a nonblank static/React first paint and a bundled native Glaze launch surface so startup communicates progress immediately;
...
The shell starts from a tiny bundled local Glaze launch document so a native first frame is available before network navigation completes. Local navigation is restricted to Tauri's own bounded application origins (`tauri://localhost` and the platform-local `tauri.localhost` forms), then the launch script immediately replaces that document with the canonical HTTPS Memos origin. After that transition, the same exact-host HTTPS navigation rule remains authoritative. `about:blank` is also permitted. Arbitrary remote hosts, HTTP Memos navigation, host-suffix lookalikes, and local-origin lookalikes remain denied. Unrequested new webview windows remain denied, no native IPC commands are intentionally exposed to remote Memos content, and the canonical server authorization/data-protection boundary remains authoritative. Android Stable distribution requires protected release signing; debug APK artifacts remain acceptance-only and are not represented as Stable packages.
...
- background/idle session continuity through the 30-day rotating refresh session rather than a false logout when a 15-minute access token expires;
- immediate Glaze startup feedback before remote identity/profile requests finish;
- a larger phone reading scale without returning to the previously oversized Compact spacing.

## Automated evidence

The frontend regression suite includes `web/tests/goreecloud-glaze-adaptive.test.ts`, `web/tests/goreecloud-mobile-session-startup.test.ts`, `web/tests/goreecloud-security-hardening.test.ts`, and `web/tests/goreecloud-application-identity.test.ts`. Together they verify that the adaptive layer is loaded after the base Glaze layers, the four official adaptive ranges are present, the effective Compact phone reading base is raised to 20 pixels, Compact readability/touch/safe-area requirements remain present, Compact density does not collapse practical interaction targets, memo metadata remains semantic and touch-sized, editor actions remain sticky and safe-area-aware on Compact, authentication uses a scrollable dynamic viewport, transient refresh failures do not unconditionally clear the session, definitive auth failure remains tied to an explicit `Unauthenticated` result, Android-relevant foreground/reconnect signals are covered, the web and React roots provide a startup surface instead of a blank first frame, the native shell starts from a bounded local launch document before transitioning to the canonical HTTPS origin, semantic controls remain inside adaptive focus/target treatment, Settings help remains viewport-safe and keyboard accessible, portaled Overlay surfaces retain Glaze adaptive and resilience coverage, shared Tabs retain roving keyboard semantics and Compact containment, the browser shell cannot recreate arbitrary instance script/style injection, unsafe branding URLs fail closed, profile customization remains bounded to local GoreeCloud assets, Wardveil presentation remains tied to evidenced protections, and web/PWA/Tauri/Linux/Android application identity remains anchored to the canonical Memos icon source.
...
The Compact interaction/viewport implementation checkpoint at source head `3537c87741a4e8120c3901a2c02779d1d8b5687f` passed the six permanent workflows: Frontend Tests #388 (`32197709405`), Backend Tests #79 (`32197709313`), GoreeCloud Security #63 (`32197709567`), GoreeCloud Container #354 (`32197710147`), Upgrade Smoke #73 (`32197709493`), and GoreeCloud Memos Clients #50 (`32197709435`). The client workflow rebuilt and uploaded both the Linux packages and ARM64 debug APK from the same exact implementation head. Stable-only release/promotion paths remained gated off.

The Android inactivity-session, first-paint startup, and Compact-readability implementation checkpoint is source head `8c75118dbb166dbc834cc6bd5cf2890223542d6b`. It adds the bounded session-recovery logic, native/web startup surfaces, 20-pixel effective Compact reading scale, and `goreecloud-mobile-session-startup.test.ts`. This checkpoint and the documentation commit that follows it require a fresh exact-head six-workflow validation matrix before source handoff; this paragraph does not claim validation merely because the source changes are committed.
...
- Compact Android device: launcher icon; immediate launch surface; cold and warm launch responsiveness; Memos feed; 20-pixel Compact reading scale and larger memo/Home text; drawer; new-memo composer with labels; sticky Save/Cancel behavior with the software keyboard open; timestamp/detail and visibility touch targets; Trash with Delete all; Settings; Members; Notifications; Wardveil account group; tabs; switches; menus; selects; dialogs; sheets; sign-in/sign-up/setup keyboard resizing and footer reachability; local profile branding; safe areas; and dynamic-viewport behavior. Leave the app backgrounded beyond the 15-minute access-token lifetime and confirm it resumes still signed in. Repeat with airplane mode or a Wi-Fi/mobile-data handoff during resume and confirm the app preserves the session, shows a recoverable loading/error state rather than sign-in, and recovers when connectivity returns.
- Linux desktop client: application icon, immediate launch surface, feed/grid, Archive, Trash, Attachments, Settings, About, Wardveil account group, local profile branding, light/dark appearance, resize/maximize/restore, pointer/keyboard focus behavior, tab keyboard navigation, and portaled overlay presentation.
- Web/PWA: canonical favicon/PWA icon identity, responsive breakpoints, nonblank first paint, Wardveil account group, light/dark appearance, keyboard access, tab keyboard navigation, portaled overlay resilience, dynamic-viewport authentication/composer behavior on narrow windows, and the same canonical session behavior used by the native shells.
