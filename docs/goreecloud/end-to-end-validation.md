...
The GoreeCloud Notes repository now has broad automated unit, component, build, isolated-container, authenticated API, attachment-binary, and restart-persistence smoke coverage, but the **first-release deployed end-to-end acceptance gate is not yet complete**.

The stable-release gate still requires validation of the deployed stable-candidate application as a complete user-facing system. Existing frontend tests, container health checks, and the isolated authenticated persistence smokes are strong supporting evidence; they do not replace browser-driven validation of the full GoreeCloud Notes workflow, attachment interaction through the deployed interface and private publication path, export, or real-device behavior on the deployed application.
...
- Attachment placement, URLs, media metadata, library behavior, upload helpers, and JSON export metadata preservation.
...
- Frontend Tests run `31787838500`; and
- GoreeCloud Container run `31787838506`.

That head also extends the full-library JSON export so attachment records preserve the normalized photo/video and motion-media metadata already exposed by the API. Attachment binary content remains intentionally excluded from the JSON export.

The later **validation-harness head** `5aa3fdd05cbc6c110f19009829ec728783106953` changes the isolated persistence smoke rather than application runtime source. On that exact head:

- Frontend Tests run `31789535244` passed; and
- GoreeCloud Container run `31789535238` passed the complete isolated container path, including both restart-persistence smokes and the attachment/state assertions below.

The original smoke in `scripts/goreecloud-notes-ci-smoke.sh` now successfully:
...
- creates a deterministic text attachment linked to that private note through the real `/api/v1/attachments` endpoint;
- reads the attachment bytes through the canonical authenticated `/file/attachments/...` route and verifies the exact payload before restart;
...
- authenticates again after restart;
- reads the same memo back and verifies the unique content marker survived; and
- reads the same attachment again through the canonical file route and verifies the exact binary payload survived the restart.

The attachment assertion deliberately uses the application file-serving route instead of reading a storage file directly. For the isolated database-backed validation configuration, this proves authenticated attachment binary create/read/restart/read persistence through the real Notes API and file-serving stack.
...
This is stronger evidence than a health-only or source-metadata check, and attachment binary persistence is now part of the isolated restart evidence. The smokes still do not claim that browser controls were exercised, that Trash restore was completed through the deployed user interface, that attachment upload/open/download was exercised through the deployed browser and private publication path, or that the deployed service was restarted with a complete production-like user state.
...
The authenticated API restart smokes now prove considerably more than a health-only container check, including exact Markdown/checklist content, source-derived label recognition, GoreeCloud color metadata, actual pinning, actual upstream Archive state, the GoreeCloud Archive-to-Trash mutation state, and attachment binary persistence through the canonical authenticated file route across container restart. They still do not prove that a real authenticated user can complete the entire first-release workflow through the browser against the deployed private service. In particular, they do not by themselves validate:
...
- attachment upload/open/download through the deployed UI and private Caddy/DNS/NetBird publication path;
...
The isolated harness now proves binary persistence through the canonical authenticated API/file route. Deployed acceptance still requires the user-facing path:

- Upload an approved test attachment through the deployed Notes interface.
- Verify the attachment appears on the note.
- Open or download the attachment through the intended deployed interface and private publication path.
- Reload the application and verify the attachment remains associated with the note and its bytes remain retrievable.
...
- Verify the JSON identifies the GoreeCloud Notes export schema/version and preserves the documented note state and attachment metadata fields, including the normalized photo/video and motion-media metadata represented by the current schema.
- Confirm documented exclusions such as attachment binary payloads, comments, and reactions are not misrepresented as included.

### Restart persistence

The isolated authenticated smokes now prove that private note content, exact Markdown checklist syntax, source-derived label recognition, GoreeCloud color metadata, actual pinned state, actual upstream Archive state, the GoreeCloud Archive-to-Trash state transition, and a linked attachment's exact binary payload through the canonical authenticated file route survive actual container restarts against the persistent data bind mount. The deployed gate still requires the broader user-visible and complete application state below.
...
- Verify notes, titles, edited Markdown content, checklist state, labels, actual pinning, colors, attachments and their retrievable bytes, actual Archive/Trash workflow state, and searchability remain intact.
...
Application restart persistence, including attachment binary persistence, is not a backup/restore test.
