...
CONTAINER_NAME="${GOREECLOUD_MEMOS_CONTAINER:-goreecloud-memos}"
...
wait_healthy() {
  ...
  docker logs "$CONTAINER_NAME"
  echo "Timed out waiting for GoreeCloud Memos after restart" >&2
  return 1
}
...
echo "Authenticated note and attachment persistence survived the GoreeCloud Memos restart."
