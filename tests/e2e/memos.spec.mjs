...
test("pinning retains manual order across reload", async ({ page }) => {
  await page.goto("/web/");
  await captureMemo(page, { content: "First pinned memo", labels: "Work" });
  await captureMemo(page, { content: "Second pinned memo", labels: "Ideas" });
  await page.locator(".memo-card", { hasText: "First pinned memo" }).getByRole("button", { name: "Pin", exact: true }).click();
  await page.locator(".memo-card", { hasText: "Second pinned memo" }).getByRole("button", { name: "Pin", exact: true }).click();
  await page.locator(".memo-card", { hasText: "Second pinned memo" }).getByRole("button", { name: "Move pin up", exact: true }).click();
...
test("presentation mode is keyboard accessible and persists across reload", async ({ page }) => {
  await page.goto("/web/");
  await captureMemo(page, { content: "Presentation memo" });
...
test("Archive and Trash are recoverable before explicit permanent deletion", async ({ page }) => {
  ...
  await page.getByRole("navigation", { name: "Memo location" }).getByRole("button", { name: "Trash", exact: true }).click();
  card = page.locator(".memo-card", { hasText: "Lifecycle memo" });
  ...
  await page.evaluate(async (databaseVersion) => {
    await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        tx.objectStore("memos").put({ schemaVersion: databaseVersion, id: `legacy-${databaseVersion}`, title: `Legacy v${databaseVersion} memo`, content: `Preserved through v${databaseVersion} migration`, createdAt: "2026-09-17T10:00:00.000Z", updatedAt: "2026-09-17T10:00:00.000Z", state: "active", pinned: databaseVersion === 2, archivedAt: null, trashedAt: null, restoreState: null });
  ...
  await page.evaluate(async () => {
    await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        const base = { schemaVersion: 3, title: "", color: null, state: "active", pinned: false, pinOrder: null, archivedAt: null, trashedAt: null, restoreState: null };
...
async function readManagedSnapshot(page) {
  return page.evaluate(async () => new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      const result = {};
      const reads = [["memos", tx.objectStore("memos").getAll()], ["labels", tx.objectStore("labels").getAll()], ["memoLabels", tx.objectStore("memoLabels").getAll()]];
      let remaining = reads.length;
      for (const [key, read] of reads) {
        read.onerror = () => reject(read.error);
        read.onsuccess = () => { result[key] = read.result; remaining -= 1; if (remaining === 0) { result.version = db.version; db.close(); resolve(result); } };
      }
    };
  }));
}
...
test("opening schema v4 migrates an existing schema v1 memo", async ({ page }) => {
  await seedLegacyMemo(page, 1); await page.goto("/web/");
  await expect(page.locator(".memo-card", { hasText: "Preserved through v1 migration" })).toBeVisible();
  const snapshot = await readManagedSnapshot(page); const record = snapshot.memos.find((memo) => memo.id === "legacy-1");
  expect(snapshot.version).toBe(4); expect(record.schemaVersion).toBe(4); expect(record.labelIds).toEqual([]);
});

test("opening schema v4 migrates an existing schema v2 memo and preserves pin state", async ({ page }) => {
  await seedLegacyMemo(page, 2); await page.goto("/web/");
  await expect(page.locator(".memo-card", { hasText: "Preserved through v2 migration" })).toBeVisible();
  const snapshot = await readManagedSnapshot(page); const record = snapshot.memos.find((memo) => memo.id === "legacy-2");
  expect(record.schemaVersion).toBe(4); expect(record.pinned).toBe(true); expect(Number.isSafeInteger(record.pinOrder)).toBe(true); expect(record.labelIds).toEqual([]);
});

test("opening schema v4 migrates v3 label names into stable managed labels and memo-label relations", async ({ page }) => {
  await seedSchemaV3Labels(page); await page.goto("/web/");
  ...
  const snapshot = await readManagedSnapshot(page);
  expect(snapshot.version).toBe(4); expect(snapshot.labels).toHaveLength(3); expect(snapshot.memoLabels).toHaveLength(4);
  const work = snapshot.labels.find((label) => label.nameKey === "work");
  expect(work).toBeTruthy(); expect(work.name).toBe("Work"); expect(typeof work.id).toBe("string"); expect(work.id.length).toBeGreaterThan(0);
  const first = snapshot.memos.find((memo) => memo.id === "v3-a"); const second = snapshot.memos.find((memo) => memo.id === "v3-b");
  expect(first.schemaVersion).toBe(4); expect(second.schemaVersion).toBe(4); expect(first.labelIds).toContain(work.id); expect(second.labelIds).toContain(work.id); expect(first.labels).toEqual(["Work", "Ideas"]); expect(second.labels).toEqual(["Work", "Research"]);
});
